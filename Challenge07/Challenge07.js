...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (x.yearsOfExperience <= 1 || (x.firstName == "null" || x.firstName == "") && (x.lastName ==
            "null" || x.lastName == "")) {
...
const classesAvg = (data) => {
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let j = 0; j < grade.classes.length; j++) {
            const x = grade.classes[j];
            let sum = 0;

            for (let q = 0; q < x.classScores.length; q++)
                sum += x.classScores[q];
            x.avg = Math.floor(sum / x.classScores.length);
        }
    }
    return data;
};
